...
const formatoPesosColombiano = (valor) => {
  ...
  // Obtener certificados de la API (ACTUALIZADO)
  useEffect(() => {
    const fetchCertificados = async () => {
      try {
        setLoading(true);
        
        let url = 'https://webhook-ecaf-production.up.railway.app/api/certificados';
        
        // Si no es admin/teacher, usar endpoint específico por email
        if (userData && userData.email && !isAdminOrTeacher()) {
          url = `https://webhook-ecaf-production.up.railway.app/api/certificados/usuario/${encodeURIComponent(userData.email)}`;
        }
        ...
        // Los datos ya vienen filtrados si es un usuario normal
  ...
  useEffect(() => {
    ...
    if (busqueda.trim()) {
      ...
      resultado = resultado.filter(cert =>
        ...
        (cert.referencia && cert.referencia.toLowerCase().includes(busquedaLower)) ||
  ...
  // FUNCIÓN SIMPLIFICADA: Solo usa el valor de la base de datos
  const getValorCertificado = (certificado) => {
    if (!certificado) return 0;
    
    // Usar únicamente el valor de la base de datos
    return Number(certificado.valor) || 0;
  };

  const getCertificadoPaymentLink = (certificado) => {
    if (!certificado) return null;
    
    const tipoCertificado = certificado.tipo_certificado?.toLowerCase() || '';
    let url = '', label = '';
                            ...
                            <TableRow key={certificado.id} hover>
                              ...
                              <TableCell>
                                <Typography sx={{ fontWeight: 'medium', color: '#CE0A0A' }}>
                                  {formatoPesosColombiano(getValorCertificado(certificado))}
                                </Typography>
                              </TableCell>
                              <TableCell>
                                <Box sx={{ display: 'flex', gap: 1 }}>
                                  {(!certificado.estado ||
                                    ...
                                    getValorCertificado(certificado) > 0 && (
                                    <Button
                                      variant="contained"
                                      size="small"
              ...
              <>
                ...
                <Box sx={{ backgroundColor: alpha('#CE0A0A', 0.03), p: 2, borderRadius: 1, mb: 3 }}>
                  ...
                  <Typography variant="h6" sx={{ color: '#CE0A0A', fontWeight: 'bold' }}>
                    {formatoPesosColombiano(getValorCertificado(certificadoSeleccionado))}
                  </Typography>
                </Box>
                ...
                {getCertificadoPaymentLink(certificadoSeleccionado)}
              </>
            )}
          </DialogContent>
        </Dialog>
